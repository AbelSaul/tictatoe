...
array=( "" "" "" "" "" "" "" "" "")
  ...
  tic_board () {
   ...
   #Muestra esta pantalla de opciones al usuario
  ...
  CHOISE () { 
   ...
   PLAYER_NAME () {
   if [ -z $USER1 ]; then # verdadero si $USER1 es nulo o vacio
    read -e -p "Introduce el nombre del primer jugador: " USER1 #Pide el nombre del primer usuario
   fi
   ...
   if [ -z $USER1 ] ; then #Si el nombre del usuario  sigue siendo vacio
     echo -e "${COLOR_RED}El nombre del jugandor no puede estar vacío"  #Nos muestre el mensaje
     PLAYER_NAME    #Llamar a la funcion otra vez para que el usuario ponga su nombre
   elif [ -z $USER2 ]; then
...
# Main Program
tic_board   #Llama a la funcion tic_board
echo -e "${DEFAULT_COL}Bienvenido al juego Tic Tac toe"  #-e nos permite hacer salto de linea
echo -e "La reglas son, introducir un número en la caja entre 0 a 8"
read -n 1 -p "Para continuar escribe y : " y  #Aqui pide una variable de entrada  que sera almacenado en la variable y -n para el numero de caracteres
echo -e "\n"
sleep 1
    ...
    if  [ "$y" == "y" ]  ||  [ "$y" == "Y" ]; then #Aqui se puede dar el caso que si es mayuscula o minuscula  entre al if
     clear #Limpia la pantalla
     echo -e "${COLOR_BLACK_BOLD}"
     PLAYER_NAME #Llama a la funcion PLAYER_NAME
    else
     echo -e "\nGracias por usarlo !!"
     exit 
    fi
 ...
 #Una vez ya registrados sus numeros      
 tic_board #Llamamos a la funcion tic_board
  while :
   do
    ((i++))
    value=`expr $i % 2` #Esto para el truquito de turno segun
    if  [ "$value" == "0" ]; then
     USER_NO=$USER1
